...
const { EmailSender } = require('./smsSender.js')
...
class ConnectionManager {
    ...
    async initializeSMSSender(smsConfig, clientId) {
        ...
        if (this.smsCache.get(clientId)) {
            return;
        }

        if (!smsConfig) {
            const clientList = await loadClientConfigs();
            smsConfig = clientList.find(client => client.ID === clientId)?.EMAIL;
            if (!smsConfig) {
    ...
    async initializeSequelize(dbConfig, clientId) {
        ...
        let db = await require('../../models/index.js')(sequelize, Sequelize, clientId);
