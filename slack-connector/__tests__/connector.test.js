...
jest.mock('../models', () => {
  ...
  return {
    ...
    __getMockTransaction: () => ({
      // Helper for fresh transaction mock
      commit: jest.fn().mockResolvedValue(undefined),
      rollback: jest.fn().mockResolvedValue(undefined),
      LOCK: { UPDATE: 'UPDATE' },
    }),
...
describe('Slack Connector - processMessage Function', () => {
  ...
  it('should update status to failed and ACK if Slack sending fails', async () => {
    ...
    expect(Notification.update).toHaveBeenCalledTimes(1);
    expect(Notification.update).toHaveBeenCalledWith({ status: 'failed', connectorResponse: slackError.error }, { where: { id: mockRecord.id } });
  ...
  it('should NACK (no requeue) on unexpected error during processing or final update', async () => {
    ...
    expect(Notification.update).toHaveBeenNthCalledWith(
      1,
    ...
    expect(logger.error).toHaveBeenCalledWith(
      expect.stringContaining('Unexpected error during processing or final DB update'),
