...
module.exports = (RABBITMQ_URL,rabbitConfig) => {
    ...
    async function processMessage(msg, channel) {
        ...
        try {
            ...
            const { clientId, messageId , templateId, content, destination,from } = notificationData;
    
            if (!clientId||!messageId ||!content||!content.subject||!content.body||!content.fromEmail ||!destination ) {
            ...
            try {
                ...
                let result=await emailConnect.sendEmail({to:destination,subject: content.subject,body: content.body,undefined,from:content.fromEmail}); 
                ...
                logger.error(`Email send failed. Updating status to 'failed'`, { messageId, dbId: notificationRecord.id, error: processingError.message });
                await dbConnect.Notification.update(  // access Notification model dynamically
