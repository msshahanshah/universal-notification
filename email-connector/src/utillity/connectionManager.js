...
class ConnectionManager {
    ...
    async initializeEmailSender(emailConfig, clientId) {
        ...
        logger.debug(`[${clientId}] Email config:`, { provider: emailConfig.AWS ? 'AWS' : emailConfig.SENDGRID ? 'SendGrid' : 'Other' });
    ...
    async getRabbitMQ(clientId) {
        ...
        if (!rabbit) {
            ...
            logger.debug(`RabbitMQ cache stats: size=${this.rabbitCache.size}, max=${this.rabbitCache.max}`);
        }
        return rabbit;
    }
