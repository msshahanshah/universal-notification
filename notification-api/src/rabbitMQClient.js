...
module.exports = (config) => {
  ...
  async function connectRabbitMQ() {
    ...
    try {
      ...
      // Assert exchange
      await channel.assertExchange(config.exchange.name, config.exchange.type, { durable: true });

      // Setup service queues if provided
      if (config.services) {
        for (const service of config.services) {
          await channel.assertQueue(service.QUEUE_NAME, { durable: true });
          await channel.bindQueue(service.QUEUE_NAME, config.exchange.name, service.ROUTING_KEY);
        }
      }
