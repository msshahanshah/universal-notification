...
const verifyVariables = (messageContent, variables) => {
  ...
  const extracted = [...new Set(matches.map((m) => m[1].trim()))];
  const payloadVars = variables.map((v) => v.name.trim());

  const allMatched = payloadVars.every((v) => extracted.includes(v)) && extracted.every((v) => payloadVars.includes(v));
...
const getTemplates = async (clientId, queryParams) => {
  const { limit = 10, page = 1, service = null, name = null, templateId = null } = queryParams;
  let dbConnect = await global.connectionManager.getModels(clientId);
