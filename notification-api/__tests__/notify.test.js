...
jest.mock('../models', () => {
  ...
  return {
    ...
    Sequelize: {
      // Mock Sequelize static properties/classes if needed
      Op: {},
    },
...
describe('POST /notify Endpoint', () => {
  ...
  it('should return 202, create DB record (pending), and publish message for valid request', async () => {
    const response = await request(app).post(BASE_URL).send(validPayload).expect('Content-Type', /json/).expect(202);
  ...
  it('should return 500 if database create fails', async () => {
    ...
    const response = await request(app).post(BASE_URL).send(validPayload).expect('Content-Type', /json/).expect(500);
  ...
  it('should return 409 if database create fails with unique constraint error', async () => {
    ...
    const response = await request(app).post(BASE_URL).send(validPayload).expect('Content-Type', /json/).expect(409); // Expect Conflict status
  ...
  it('should return 500 and update DB status to failed if publishing fails', async () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith(
      expect.stringContaining("Updated notification status to 'failed' due to publish error"),
      expect.any(Object)
    );
  });
